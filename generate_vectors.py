...
def generate_vectors(db):
    ...
    for row in LFF_Keywords.select():
        ...
        for word in words:
            ...
            # Only increment if the ticket contains the  keyword or not (DF)
    ...
    for row in LFF_Keywords.select():
        trouble_ticket = dict((keyword, 0) for keyword in document_frequency_dict.keys())
        words = nltk.word_tokenize(row.description)
        ...
        for word in words:
            ...
            if word not in words_found:
                words_found.add(word)
                trouble_ticket[word] = document_frequency_dict[word]
        words_found.clear()
        list_trouble_ticket_dicts.append(trouble_ticket)
